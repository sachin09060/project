...
def open_win():
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="HOSPITAL DRUG BILLING SYSTEM").grid(row=0, column=0, columnspan=3)
...
def blue():
    global st, c, cur, named, add, t, vc_id
    ...
    for i in cur:
        if vc_id.get() != '' and int(vc_id.get()) == i[2]:
            ...
            Label(st, text=add, width=20).grid(row=2, column=1)
...
def make_bill():
    global t, c, B, cur, st, names, qty, sl, named, add, name1, add, det, vc_id
    price = [0.0] * 10
    q = 0
    det = ['', '', '', '', '', '', '', '']
    det[2] = str(sl)
    ...
    m += "===============================================\n"
    m += "                                  No :%s\n\n" % det[5]
    m += " HOSPITAL DRUG BILLING SYSTEM\n"
    m += " VIJAYNAGAR BRANCH\n\n"
    m += "-----------------------------------------------\n"
    if t == 1:
        m += "Name: %s\n" % named
...
def again():
    ...
    root.title('KNOWX DRUG COMPANY')
    Label(root, text='HOSPITAL DRUG BILLING SYSTEM').grid(row=0, column=0, columnspan=5)
    Label(root, text="VIJAYNAGAR BRANCH").grid(row=1, column=0, columnspan=5)
...
def open_cus():
    ...
    apt = Tk()
    apt.title("Interface")
    Label(apt, text="*** HOSPITAL DRUG BILLING SYSTEM ***", bg='blue', fg='white').grid(row=0, column=0)
